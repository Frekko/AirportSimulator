...
namespace Airport_Visualisator
{
    public partial class Form1 : Form
    {
        ...
        public void Network() //связывается с первой частью визуализатара (логической)
        {
            ...
            while (true)
            {
                string planeString = CommonUtil.MakeRequest(String.Format("{0}/GetAllPlanes", ServiceStrings.Vis)); //запрашивает самолёты
//                planesList = JsonConvert.DeserializeObject<List<Tuple<int, Point, int, int, int>>>(planeString);
                string vehicleString = CommonUtil.MakeRequest(String.Format("{0}/GetAllVehicles", ServiceStrings.Vis)); //запрашивает машинки
//                vehicleList = JsonConvert.DeserializeObject<List<Tuple<int, Point, int>>>(vehicleString);
                string passengerString = CommonUtil.MakeRequest(String.Format("{0}/GetAllPassengers", ServiceStrings.Vis)); //запрашивает пассажиров
